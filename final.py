...
train_logits = model.call(all_x)
true_idx = tf.equal(tf.argmax(train_logits, 1), tf.argmax(y_train[:640], 1))
false_idx = tf.not_equal(tf.argmax(train_logits, 1), tf.argmax(y_train[:640], 1))
...
def regu_train(model, regu_model, train_inputs, train_labels, delta_x):
    ...
    for x in range(0, train_inputs.shape[0], model.batch_size):
        ...
        batch_delta_x = delta_x[x: x + model.batch_size, :]
...
def regu_test(regu_model, test_inputs, test_labels):
    acc = []
    for x in range(0, test_inputs.shape[0], model.batch_size):
        batch_inputs = test_inputs[x: x + model.batch_size, :]
        batch_labels = test_labels[x: x + model.batch_size]
...
for i in range (epochs):
    ...
    regu_test_acc = regu_test(regu_model, test_all_x, y_test[:128])
    print(regu_test_acc)
